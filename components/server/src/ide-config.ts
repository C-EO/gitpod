...
export class IDEConfigService {
    ...
    private reconcile = debounce(async () => {
        try {
            ...
            try {
                const nightlyImageDigest = await this.resolveNightlyImageDigest();
                if (value && value.ideImageAliases['code-nightly'] !== nightlyImageDigest) {
                    value.ideImageAliases['code-nightly'] = nightlyImageDigest;
                }
            } catch (e) {
                console.error('ide config: cannot resolve nightly image: ', e);
            }
    ...
    private resolveNightlyImageDigest() {
        return new Promise<string>((resolve, reject) => {
            cp.exec('oci-tool --timeout 30s resolve name eu.gcr.io/gitpod-core-dev/build/ide/code:nightly', (error, imageDigest) => {
                if (error) {
                    return reject(error);
                }

                if (!imageDigest) {
                    throw new Error('Cannot resolve eu.gcr.io/gitpod-core-dev/build/ide/code:nightly image');
                }
