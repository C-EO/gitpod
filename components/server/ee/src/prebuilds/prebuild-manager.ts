    ...
    async startPrebuild(ctx: TraceContext, { contextURL, cloneURL, commit, branch, project, user }: StartPrebuildParams): Promise<StartPrebuildResult> {
        ...
        try {
            ...
            if (!!existingPB) {
                ...
                const isFailed = existingPB.state === 'aborted' || existingPB.state === 'timeout' || !!existingPB.error;
                // If the prebuild is based on an outdated project config, we want to retrigger it.
